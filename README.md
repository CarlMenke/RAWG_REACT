...
```
REACT_APP_RAWG_KEY=123456789
```

Be sure and add your `.env` file to your `.gitignore` simply by typing ".env" on line 3 in the `.gitignore` file!

Restart your react dev server and can now utilize the key with the following example: `process.env.REACT_APP_RAWG_KEY`

Note: You can name your key *anything you want* as long as it starts with `REACT_APP` and is in **SCREAMING_SNAKE_CASE**.
...
- Once these Links are finished, you are done with the Header component!
...
- Pull request utilizing this template: [PR Guidelines](https://github.com/SEI-R-11-8/template_pull_request)
